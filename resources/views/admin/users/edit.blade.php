...
<div class="grid form-grid"><label>Nome<input name="name" value="{{ old('name',$managedUser->name) }}" required></label><label>E-mail<input type="email" name="email" value="{{ old('email',$managedUser->email) }}" required></label><label>Cargo<select name="role_id"><option value="">Sem cargo</option>@foreach($roles as $role)<option value="{{ $role->id }}" @selected((string)old('role_id',$managedUser->role_id)===(string)$role->id)>{{ $role->name }}</option>@endforeach</select></label><label>Departamento<select name="department_id"><option value="">Sem departamento</option>@foreach($departments as $department)<option value="{{ $department->id }}" @selected((string)old('department_id',$managedUser->department_id)===(string)$department->id)>{{ $department->name }}</option>@endforeach</select></label><label>WhatsApp da equipe (DDD + número)
    <input name="whatsapp" type="tel" inputmode="tel" maxlength="30" placeholder="(15) 99999-8888" value="{{ old('whatsapp', $managedUser->whatsapp) }}">
    <small class="muted">Receberá aviso quando um cliente responder a um ticket sob responsabilidade deste usuário.</small>
</label><label class="toggle-card"><input type="hidden" name="whatsapp_reply_enabled" value="0"><input type="checkbox" name="whatsapp_reply_enabled" value="1" @checked(old('whatsapp_reply_enabled', $managedUser->whatsapp_reply_enabled))><span><b>Notificar respostas dos clientes por WhatsApp</b><small>Além do e-mail e do sininho, quando houver número cadastrado e automação ativa.</small></span></label><label class="toggle-card"><input type="checkbox" name="active" value="1" @checked(old('active',$managedUser->active))><span><b>Usuário ativo</b><small>Desmarque para impedir novos acessos.</small></span></label></div></article>
