...
<div class="responsive-table desktop-admin-table"><table class="admin-table"><thead><tr><th>Usuário</th><th>Cargo</th><th>Departamento</th><th>Confirmação</th><th>Status</th><th></th></tr></thead><tbody>
@forelse($users as $user)<tr><td><b>{{ $user->name }}</b><small>{{ $user->email }}</small></td><td>{{ $user->role?->name ?? 'Sem cargo' }}</td><td>{{ $user->department?->name ?? 'Sem departamento' }}</td><td><span class="pill {{ $user->hasVerifiedEmail()?'ok':'off' }}">{{ $user->hasVerifiedEmail()?'E-mail confirmado':'Não confirmado' }}</span></td><td><span class="pill {{ $user->active?'ok':'off' }}">{{ $user->active?'Ativo':'Inativo' }}</span></td><td class="right"><a class="secondary-button compact" href="{{ route('admin.users.edit',$user) }}">Editar</a></td></tr>@empty<tr><td colspan="6" class="empty">Nenhum usuário cadastrado.</td></tr>@endforelse
</tbody></table></div>
...
<article class="mobile-admin-card">
    ...
    <div class="mobile-admin-meta"><div><small>Cargo</small><strong>{{ $user->role?->name ?? 'Sem cargo' }}</strong></div><div><small>Departamento</small><strong>{{ $user->department?->name ?? 'Sem departamento' }}</strong></div><div><small>Confirmação</small><strong>{{ $user->hasVerifiedEmail()?'E-mail confirmado':'Não confirmado' }}</strong></div></div>
