...
@forelse($departments as $department)
    ...
    <article class="panel department-card" data-department-card>
        <button type="button" class="department-summary" data-department-toggle aria-expanded="false">
            <span class="department-title"><strong>{{ $department->name }} @if($department->isTriage())<span class="department-system-badge">Padrão</span>@endif @if(($unseenCounts[$department->id] ?? 0) > 0)<span class="department-new-count">{{ $unseenCounts[$department->id] }} {{ $unseenCounts[$department->id] === 1 ? 'novidade' : 'novidades' }}</span>@endif</strong><small>{{ $department->isTriage() ? 'Caixa padrão do sistema' : ($department->active ? 'Ativo' : 'Inativo') }}</small></span>
            @if($canViewTickets)
        ...
        <div class="department-details" data-department-details hidden>
            ...
            @if($canManage)
                ...
                @unless($department->isTriage())<div class="department-admin-actions"><a href="{{ route('admin.departments.edit',$department) }}">Editar nome/status do departamento</a></div>@endunless
            @endif
        </div>
    </article>
@empty
