...
class IntegrationApiV1Test extends TestCase
{
    ...
    private function status(string $key, string $name, string $category = 'open'): Status
    {
        return Status::firstOrCreate(
            ['system_key' => $key],
            ['name' => $name, 'category' => $category, 'color' => '#6D28D9', 'active' => true]
        );
    }

    private function integration(string $name, string $token, ?Department $department = null, bool $active = true): ConnectedSystem
    {
        $company = Company::create(['name' => 'Legacy '.uniqid(), 'active' => true]);
    ...
    public function test_user_sees_only_own_tickets_manager_sees_all_for_same_key_and_never_another_key(): void
    {
        ...
        $user = $this->withHeaders($this->headers('token-a', '1', 'user'))->getJson('/api/v1/tickets');
        $user->assertOk()->assertJsonCount(1, 'data')->assertJsonPath('data.0.title', 'A1');
    ...
    {
        $this->integration('Inativa', 'token-off', null, false);
    ...
    public function test_public_comment_and_activity_never_expose_internal_note(): void
    {
        ...
        $this->withHeaders($this->headers('token-franca', '153'))->postJson('/api/v1/tickets/'.$ticket->number.'/comments', ['body' => 'Retorno do cliente', 'external_message_id' => 'msg-1'])->assertCreated();
