...
{
    public function store(Request $request, Ticket $ticket, TicketEventRecorder $events, WebhookDispatcher $webhooks)
    {
        $actor = $request->user();
        ...
        if ($data['visibility'] === 'public') {
            $webhooks->queue($ticket->fresh(['status', 'system']), 'ticket.comment.created', [
                'comment' => [
                    'id' => $comment->id,
                    'body' => $comment->body,
