...
- **Push nesta fase:** registra o alerta no sininho e mostra uma notificação do navegador enquanto a aplicação está aberta, se o usuário concedeu permissão no dispositivo. **Não equivale a Web Push em segundo plano:** receber avisos com o navegador ou aplicativo fechado requer um serviço de assinatura Web Push com VAPID, infraestrutura de envio e testes por navegador/dispositivo, ainda pendente.
...
- [ ] Garantir que a página da caixa destaque tickets novos/atualizados até a confirmação explícita; navegador fechado **não** recebe Web Push de fundo nesta fase.
